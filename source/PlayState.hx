...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		PlayStateChangeables.Modcharts = _camsave.data.modcharts;

		removedVideo = false;

		#if FEATURE_LUAMODCHART
		if (PlayStateChangeables.Modcharts && storyDifficulty == 2 && !_camsave.data.opponentmode)
		{
		...
		{
			if (_camsave.data.opponentmode)
			{
				dialogue = CoolUtil.coolTextFile(Paths.txt('data/songs/${PlayState.SONG.songId}/dialogue-c'));
			}
			else
			{
		...
		if (!stageTesting)
		{
			...
			if (_camsave.data.opponentmode)
			{
				dad = new Character(100, 100, SONG.player1, false);
			}
			else
			{
		...
		if (!stageTesting || !PlayStateChangeables.Optimize)
		...
		if ((_camsave.data.characters && PlayStateChangeables.Optimize) || !PlayStateChangeables.Optimize)
		...
		if ((FlxG.save.data.laneUnderlay && !PlayStateChangeables.Optimize) || !executeModchart)
		{
		...
		stepCounter = new FlxText(20, 20, 0, "", 20);
		stepCounter.setFormat(Paths.font("vcr.ttf"), 20, FlxColor.WHITE, FlxTextAlign.LEFT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		stepCounter.borderSize = 2;
		stepCounter.borderQuality = 2;
		stepCounter.scrollFactor.set();
		stepCounter.cameras = [camHUD];
		...
		if (isStoryMode)
		{
			...
			{
				case 'first-town' | 'liquated' | 'why-do-you-hate-me':
					camelliaDialogue(doof);
				case 'quaoar' | 'crystallized' | 'nacreous-snowmelt':
					camelliaDialogue(doof);
				case 'ghost' | 'ghoul' | 'ghost-vip':
					camelliaDialogue(doof);
				case 'lioness-pride':
					camelliaDialogue(doof);
				default:
	...
	{
		if (SONG.songName.toLowerCase() != "quaoar")
		{
	...
	public function generateSong(dataPath:String):Void
	{
		...
		else if (PlayState.SONG.songName.toLowerCase() == "tutorial" && storyDifficulty == 2)
		{
			FlxG.sound.playMusic(Paths.inst(PlayState.SONG.songId, true), 1, false);
			PlayState.playAsGF = true;
		...
		#else
		if (PlayState.SONG.songName.toLowerCase() == "tutorial" && PlayState.storyDifficulty == 2)
		{
			FlxG.sound.playMusic(Paths.inst(PlayState.SONG.songId, true), 1, false);
			PlayState.playAsGF = true;
		...
		if (PlayState.SONG.songName.toLowerCase() == "tutorial"
			&& _camsave.data.opponentmode
			&& (PlayState.storyDifficulty == 0 || PlayState.storyDifficulty == 1))
		{
			PlayState.playAsGF = true;
			PlayState.boyfriend.visible = false;
		}
		...
		for (section in noteData)
		{
			...
			for (songNotes in section.sectionNotes)
			{
				...
				if (!gottaHitNote && !executeModchart && !_camsave.data.characters)
					continue;
	...
	private function generateStaticArrows(player:Int):Void
	{
		for (i in 0...4)
		{
			...
			if (FlxG.save.data.middleScroll && !executeModchart && player == 0)
	...
	override public function update(elapsed:Float)
	{
		...
		if (generatedMusic)
		{
			if (songStarted && !endingSong)
			{
				...
				if (unspawnNotes.length == 0 && notes.length == 0 && !isStoryMode && songMultiplier != 1)
				{
		...
		if ((FlxG.keys.anyJustPressed([pauseBind]) #if android || FlxG.android.justReleased.BACK #end && FlxG.keys.anyJustPressed([gppauseBind]))
			&& startedCountdown
			&& canPause
			&& !cannotDie)
		{
		...
		if (generatedMusic)
		{
			...
			notes.forEachAlive(function(daNote:Note)
			{
				...
				if (!daNote.mustPress && Conductor.songPosition >= daNote.strumTime)
				{
					...
					if (!daNote.isParent && daNote.parent != null)
					{
						if (daNote.spotInLine != daNote.parent.children.length - 1)
						{
							...
							if (!daNote.isSustainNote)
							{
								if (playAsGF && !_camsave.data.opponentmode)
								{
									gf.playAnim('sing' + dataSuffix[singData] + altAnim, true);
									gf.holdTimer = 0;
								}
								else
								{
					...
					&& songStarted)
				{
					...
					{
						if (!_camsave.data.opponentmode || PlayState.SONG.songName.toLowerCase() == "lioness pride")
						{
							vocals.volume = 0;
						}
						else
	...
	function endSong():Void
	{
		...
		else
		{
			if (isStoryMode)
			{
				...
				if (SONG.songName.toLowerCase() == 'ghost halloween vip')
				{
					_camsave.data.picoUnlock = true;
					_camsave.flush();
				}
	...
	private function popUpScore(daNote:Note):Void
	{
		...
		if (daRating != 'shit' || daRating != 'bad')
		{
			...
			if (currentTimingShown != null || !_camsave.data.ratingtoggle)
			...
			switch (daRating)
			{
				...
				case 'bad':
					currentTimingShown.color = 0xFF7707;
				case 'good':
					currentTimingShown.color = 0x006EFF;
				case 'sick':
					currentTimingShown.color = 0xF9FF5C;
				case 'marvelous':
					currentTimingShown.color = 0xD76058;
			}
			...
			for (i in seperatedScore)
			{
				...
				if (_camsave.data.ratingtoggle && !PlayStateChangeables.botPlay)
					add(numScore);
	...
	function goodNoteHit(note:Note, resetMashViolation = true):Void
	{
		...
		if (!note.wasGoodHit)
		{
			...
			if (!note.isSustainNote)
			{
				if (playAsGF && _camsave.data.opponentmode)
				{
					gf.playAnim('sing' + dataSuffix[note.noteData] + altAnim, true);
					gf.holdTimer = 0;
				}
				else
				{
	...
	override function stepHit()
	{
		...
		stepCounter.text = "Step: " + curStep;

		if (SONG.songName.toLowerCase() == "tutorial")
		{
			if (storyDifficulty == 2 && tutorialRepeat == 0)
			{
				playAsGF = true;
		...
		switch (SONG.songName.toLowerCase())
		{
